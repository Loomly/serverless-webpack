...
function zip(directory, name) {
  ...
  const keepSymlinks = this.configuration.keepSymlinks();
    ...
    _.forEach(files, filePath => {
      ...
      if (!stats.isDirectory(fullPath)) {
        if (keepSymlinks) {
          ...
          zip.append(isSymbolicLink ? null : fs.readFileSync(fullPath), {
            name: filePath,
            mode: stats.mode,
            date: new Date(0), // necessary to get the same hash when zipping the same content
            type: isSymbolicLink ? "symlink" : "file",
            linkname: isSymbolicLink ? fs.readlinkSync(fullPath) : null
          });
        }
      }
    });
