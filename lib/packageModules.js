...
function zip(directory, name) {
  ...
  const preserveSymlinks = this.configuration.preserveSymlinks();
  ...
  output.on('open', () => {
    ...
    _.forEach(files, filePath => {
      ...
      const appendAsSymLink = preserveSymlinks && isSymLink;

      if (!stats.isDirectory(fullPath)) {
        zip.append(appendAsSymLink ? null : fs.readFileSync(fullPath), {
          name: filePath,
          mode: stats.mode,
          date: new Date(0), // necessary to get the same hash when zipping the same content
          type: appendAsSymLink ? "symlink" : "file",
          linkname: appendAsSymLink ? fs.readlinkSync(fullPath) : null
        });
      }
    });
